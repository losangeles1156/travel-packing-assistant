...
interface ResultViewProps {
  ...
  onStartCollab: () => Promise<string | null>;
  isSyncing: boolean;
}
        ...
        <div className="flex gap-3 items-center">
          ...
          {!collabSessionId ? (
            <button
              ...
              className="bg-indigo-600 text-white px-5 py-2.5 rounded-xl hover:bg-indigo-700 font-bold flex items-center gap-2 shadow-lg shadow-indigo-200 transition-all hover:-translate-y-0.5 active:translate-y-0"
            >
              <i className="fa-solid fa-users"></i> 開啟即時協作
            </button>
          ) : (
            <div className="flex items-center gap-2 bg-green-50 text-green-700 px-4 py-2.5 rounded-xl border border-green-200 font-bold shadow-sm">
                ...
                {scenarios.map(s => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex-shrink-0 px-3 py-1.5 rounded-full text-xs font-black border transition ${active
              ...
              <div className="flex flex-wrap items-center gap-3">
                {activeScenario && (
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={handleScenarioToggleOnly}
                      className={`px-3 py-1.5 rounded-full text-xs font-black border transition ${scenarioOnly ? 'bg-emerald-50 text-emerald-700 border-emerald-200' : 'bg-white text-slate-600 border-slate-200'
                        }`}
                    >
              ...
              <ul className="space-y-3 flex-1">
                {filteredCarryOnItems.map(item => {
                  ...
                  return (
                    ...
                    >
                      <div className={`w-5 h-5 rounded border-2 mt-1 flex items-center justify-center transition-all duration-200 ${isPacked
        ...
        <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-100 mb-8 print:shadow-none print:border-none print:p-0 print:mt-8 page-break">
          ...
          <div className="grid gap-3">
            {tasks.map(task => (
              <div
                key={task.id}
                className={`flex items-center p-4 rounded-xl border transition-all cursor-pointer no-print group ${task.completed
              ...
              >
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center mr-4 transition-colors ${task.completed
